...
#>>> dtypes: int64(2), object(1)
# 개발자 분석내용 - document 필드의 수량이 다른 필드의 수량과 틀리므로 결측 데이터 존재

#seperate title [] ------------------
print("훈련데이터 결측수량:",train_data["document"].isna().sum())
print("테스트데이터 결측수량:",test_data["document"].isna().sum())
# 훈련데이터와 테스트 데이터의 결측값을 제거한후 결측수량을 다시 확인하시오
...
# >> 훈련데이터 결측수량: 0
# >> 테스트데이터 결측수량: 0
# 개발자 분석내용 - 최초의 훈련데이터에 5개의 결측데이터가 관측되었고
#                    테스트 데이터에 3개의 결측데이터가 관측되었다 
#                     pandas의 dropna 명령으로 제거함

#seperate title [4. 중복데이터 확인 및 제거 ] ============
print(train_data["document"].count()) # 총 데이터 수량
print(train_data["document"].nunique()) # 유니크한 데이터 수량
# >>> 149995
# >>> 146182
...
# >>> 49157
# >>> 테스트 중복된 데이터의 수: 840
# 개발자 분석내용 - 테스트데이터 또한 중복 내요잉 존재하고있으나 
#                   훈련대상 데이터가 아니지만 중복 내용을 제거 하겠다.
...
# >>> 훈련 중복된 데이터의 수: 0
# >>> 테스트 중복된 데이터의 수: 0
# 개발자 분석내용 - 모든 데이터의 중복이 제거되어 중복데이터 수가 0으로표기 되었다
